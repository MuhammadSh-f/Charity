 ...
 */
const NS = "org.transfer.money.organization.net";
const factory = getFactory();

/**
 * createProjectPledge
 * @param {org.transfer.money.organization.net.CreateProjectPledge} createProjectPledge
 * @transaction
 */
function createProjectPledge(txParams) {
  ...
  return getAssetRegistry(`${NS}.ProjectPledge`)
    .then((registry) => {
      ...
      pledge.needDonation = txParams.needDonation;
      pledge.fundsRequired = txParams.fundsRequired;
      pledge.fundsReceived = txParams.fundsReceived;
      pledge.status = txParams.status;
 ...
 * @param {org.transfer.money.organization.net.SendPledgeToGovernement} SendPledgeToGovernement
 * @transaction
 */
 ...
 * @param {org.transfer.money.organization.net.ApprovePledgeFromGovernement} ApprovePledgeFromGovernement
 * @transaction
 */
...
/**
 * CreateSendMoney
 * @param {org.transfer.money.organization.net.CreateSendMoney} CreateSendMoney
 * @transaction
 */
...
/**
 * UpdateProject
 * @param {org.transfer.money.organization.net.UpdateProject} UpdateProject
 * @transaction
 */
function UpdateProject(txParams) {
  if (!txParams.pledgeId || (txParams.pledgeId && txParams.pledgeId === "")) {
    throw new Error("Invalid pledgeId!!");
  }
  if (txParams.pledgeId.fundsReceived >= txParams.pledgeId.fundsRequired) {
    txParams.pledgeId.needDonation = false;
  }
  return getAssetRegistry(`${NS}.ProjectPledge`).then((registry) =>
    registry.update(txParams.pledgeId)
  );
}
